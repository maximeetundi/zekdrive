  ...
  Future<void> loadData() async{
    ...
    if(Get.find<RideController>().ongoingTripDetails != null){
      for(int i =0 ;i<Get.find<RideController>().ongoingTripDetails!.length; i++){
        ...
        if(Get.find<RideController>().ongoingTripDetails![i].currentStatus == 'completed' && Get.find<RideController>().ongoingTripDetails![i].paymentStatus == 'unpaid'){
          PusherHelper().customerCouponAppliedOrRemoved(Get.find<RideController>().ongoingTripDetails![i].id!);
        }}}
                  ...
                  CustomMenuWidget(
                    ...
                    onPressed: () {
                      if(rideController.parcelListModel != null && rideController.parcelListModel!.data != null && rideController.parcelListModel!.data!.isNotEmpty){
                        Get.to(()=>  OngoingParcelListViewWidget(title: 'ongoing_parcel_list', parcelListModel: rideController.parcelListModel!));
