          ...
          child: GetBuilder<RideController>(
            builder: (finalFareController) {
              ...
              Column(crossAxisAlignment: CrossAxisAlignment.start,children: [
                AppBarWidget(title: 'sub_total'.tr, showBackButton: true, onBackPressed: ()=> Get.offAll(()=> const DashboardScreen())),
                ...
                Padding(padding: const EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeDefault), child: RouteWidget(
                ...
                Padding(padding: const EdgeInsets.all(Dimensions.paddingSizeDefault),
                  child: Container(padding: const EdgeInsets.fromLTRB(Dimensions.paddingSizeDefault, Dimensions.paddingSizeDefault, Dimensions.paddingSizeDefault,0),
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start,children: [
                      ...
                      PaymentItemInfoWidget(icon: Images.farePrice,title: 'fare_price'.tr,amount: finalFareController.finalFare?.distanceWiseFare??0),

                      if(!fromParcel && finalFareController.finalFare!.cancellationFee!.toDouble() > 0)
                        PaymentItemInfoWidget(icon: Images.idleHourIcon,title: 'cancellation_price'.tr,amount: finalFareController.finalFare?.cancellationFee??0),

                      if(!fromParcel && finalFareController.finalFare!.idleFee!.toDouble() > 0)
                        PaymentItemInfoWidget(icon: Images.idleHourIcon,title: 'idle_price'.tr,amount: finalFareController.finalFare?.idleFee??0),

                      if(!fromParcel && finalFareController.finalFare!.delayFee!.toDouble() > 0)
                        PaymentItemInfoWidget(icon: Images.waitingPrice,title: 'delay_price'.tr,amount: finalFareController.finalFare?.delayFee??0),

                      if(finalFareController.finalFare!.couponAmount!.toDouble() > 0)
                      PaymentItemInfoWidget(icon: Images.coupon, title: 'coupon_amount'.tr, amount: finalFareController.finalFare?.couponAmount??0,
                      ...
                      PaymentItemInfoWidget(icon: Images.discountIcon, title: 'discount_applied'.tr,
                        amount: finalFareController.finalFare?.discountAmount??0,discount: true,toolTipText: 'discount_applied_for_this_ride'.tr,
                        subTitle: 'later_admin_will_pay_you_this_amount',),

                      if(finalFareController.finalFare!.vatTax!.toDouble() > 0)
                      PaymentItemInfoWidget(icon: Images.farePrice,title: 'vat_tax'.tr,amount: finalFareController.finalFare?.vatTax??0),

                      PaymentItemInfoWidget(title: 'sub_total'.tr,amount: finalFareController.finalFare?.paidFare??0, isSubTotal: true,),
                  ],),),
                )
              ...
              builder: (tripController) {
                return Container(padding: const EdgeInsets.fromLTRB(Dimensions.paddingSizeDefault, Dimensions.paddingSizeDefault,Dimensions.paddingSizeDefault,Dimensions.paddingSizeLarge),
                  ...
                  ButtonWidget(buttonText: 'payment_received'.tr,
                    onPressed: () {
                     Get.dialog(ConfirmationDialogWidget(icon: Images.paymentIcon,description: 'are_you_sure_you_got_cash_payment_from_customer'.tr, onYesPressed: (){
                       tripController.paymentSubmit(finalFareController.finalFare!.id!, 'cash', fromSenderPayment: fromParcel);
                     },));
                  }),
                );
              }
