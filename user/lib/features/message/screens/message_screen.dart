...
class _MessageScreenState extends State<MessageScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: BodyWidget(
        appBar: AppBarWidget(title:"${'chat_with'.tr} ${widget.userName}", showBackButton: true, centerTitle: true),
              ...
              child: PaginatedListWidget(
                ...
                itemView: ListView.builder(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    ...
                    if(index !=0){
                      return ConversationBubble(message: messageController.messageModel!.data![index], previousMessage: messageController.messageModel!.data![index-1], index: index,length: messageController.messageModel!.data!.length,);
                    }else{
                      return ConversationBubble(message: messageController.messageModel!.data![index], index: index,length: messageController.messageModel!.data!.length,);
                    }
                  },
                ),
              ),
            ),
            ) : const Expanded(child: NoDataWidget(title: 'no_message_found')) : const Expanded(child: NotificationShimmer()),
...
*/
                      ...
                      messageController.isSending? SpinKitCircle(color: Theme.of(context).primaryColor, size: 40.0,) : messageController.isImagePicked ?
                      SpinKitCircle(color: Theme.of(context).primaryColor, size: 40.0,) : Padding(padding: const EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeSmall),
                        child: InkWell(onTap: (){
                          if(messageController.conversationController.text.trim().isEmpty && messageController.pickedImageFile!.isEmpty
